...
export function useRouteCalculation(
  ...
  const updateRouteForDay = useCallback(async (dayId: number | null) => {
    ...
    type Entry =
      | { kind: 'place'; lat: number; lng: number; pos: number; time: string | null; durationMinutes: number }
    ...
    const drawMorning = !!bookends && !!selectedDay && shouldDrawMorningLeg(bookends, selectedDay, edgeInfo(firstStop))
    const drawEvening = !!bookends && !!selectedDay && shouldDrawEveningLeg(bookends, selectedDay, edgeInfo(lastStop))
    ...
    if (runsWithHotel.length === 0 && drawMorning && drawEvening) {
      if (startHotelPoint && endHotelPoint && (startHotelPoint.lat !== endHotelPoint.lat || startHotelPoint.lng !== endHotelPoint.lng)) {
        runsWithHotel.push([startHotelPoint, endHotelPoint])
      }
    }
