  ...
  if (W) server.registerTool(
    'create_trip',
    {
      ...
      inputSchema: {
        ...
        routing_provider: z.enum(['osrm', 'google_maps', 'google_maps_mobile']).optional().describe('Routing provider for travel-time estimates'),
  ...
  if (W) server.registerTool(
    'update_trip',
    {
      ...
      inputSchema: {
        ...
        routing_provider: z.enum(['osrm', 'google_maps', 'google_maps_mobile']).optional(),
    ...
    async ({ tripId, title, description, start_date, end_date, currency, schedule_margin_minutes, routing_provider, routing_optimism, routing_avoid_tolls, routing_avoid_highways, routing_avoid_ferries, is_archived, cover_image, date_shift_mode }) => {
      if (isDemoUser(userId)) return demoDenied();
      ...
      const { updatedTrip } = updateTrip(tripId, userId, { title, description, start_date, end_date, currency, schedule_margin_minutes, routing_provider, routing_optimism, routing_avoid_tolls, routing_avoid_highways, routing_avoid_ferries, is_archived, cover_image, date_shift_mode }, 'user');
